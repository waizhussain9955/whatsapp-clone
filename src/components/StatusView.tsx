...
const StatusView: React.FC<StatusViewProps> = ({ statuses, onClose }) => {
    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#000000', zIndex: 2000, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative' }}>
        {currentStatus.type === 'image' ? (
          ...
          <div style={{ fontSize: '32px', color: 'white', textAlign: 'center', padding: '20px' }}>
            {currentStatus.content}
          </div>
        )}
        
        {/* Transparent tap areas to navigate */}
        <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '30%', zIndex: 5 }} onClick={() => {
          if (currentIndex > 0) { setCurrentIndex(c => c - 1); setProgress(0); }
        }} />
        <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: '30%', zIndex: 5 }} onClick={() => {
